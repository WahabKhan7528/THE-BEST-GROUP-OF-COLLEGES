  ...
  return (
    <div className="w-full mx-auto space-y-6">
        ...
        <div className="flex flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
          <div className="flex items-start sm:items-center gap-2 sm:gap-4 min-w-0 flex-1">
            ...
            <div className="min-w-0 relative">
              <h1 className="text-lg sm:text-2xl font-bold text-college-navy dark:text-white tracking-tight break-words pr-2">
                {title}
              </h1>
              {subtitle && (
          ...
          {headerActions && (
            <div className="flex-shrink-0 flex justify-end self-start sm:self-center">
              {headerActions}
            </div>
          )}
        </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-end gap-3 md:gap-4 pt-6 mt-4">
          {onCancel && (
            <PublicButton
              variant="primary"
              onClick={onCancel}
              className="w-full sm:w-auto border-2 border-white/10"
              type="button"
            >
              {cancelLabel}
            </PublicButton>
          ...
          <PublicButton
            ...
            shape="slanted"
            size="md"
            className="w-full sm:w-auto px-6 md:px-8 font-bold shadow-md transform hover:-translate-y-0.5"
            icon={SubmitIcon}
            disabled={submitting}
          >
