...
const emptySemesterRows = (count, sourceRows = [], examSystem = "semester") =>
  Array.from({ length: count }, (_, index) => {
    ...
    const subjectAssignments = (source?.subjectAssignments || [])
      .map((assignment) => ({
        subject: String(
...
const buildSemesterRowsFromClass = (
  classRoom,
  count,
  examSystem = "semester",
) => {
  if (
    ...
    classRoom.semesterSubjects.length > 0
  ) {
    return emptySemesterRows(count, classRoom.semesterSubjects, examSystem);
  }

  const fallbackSemesterNumber =
    semesterNumberFromValue(classRoom.semester) || 1;
...
const EditClass = () => {
  ...
  const activeCampusId = isSuperAdmin
    ? watch("campus") || selectedCampuses[0] || ""
  ...
  const lockedSemesterCount = semesterRows.filter(
    (row) =>
      row.status === "locked" ||
  ...
  useEffect(() => {
    const loadClass = async () => {
      try {
        ...
        const existingClass = (classRes.data.data || []).find(
          (item) => item._id === id,
        );
        if (!existingClass) {
        ...
        const classCampusId =
          existingClass.campus?._id || existingClass.campus || "";
        if (!isSuperAdmin) {
          const adminCampusId =
            currentAdmin?.campus?._id || currentAdmin?.campus || "";
          if (
            adminCampusId &&
            String(classCampusId) !== String(adminCampusId)
          ) {
            toast.error(
              "Access denied. This class is not assigned to your campus.",
            );
            navigate("/admin/classes", { replace: true });
            return;
          }
        ...
        const courseId =
          existingClass.course?._id || existingClass.course || "";
        const course = coursesRes.data.data?.find(
          (item) => item._id === courseId,
        );
        const semesterTotal =
          course?.examSystem === "annual"
        ...
        setSemesterRows(
          buildSemesterRowsFromClass(
            existingClass,
            semesterTotal,
            course?.examSystem || "semester",
          ),
        );
      } catch {
  ...
  useEffect(() => {
    ...
    if (
      selectedCourse.examSystem === "semester" ||
      selectedCourse.examSystem === "annual"
    ) {
  ...
  const selectedSubjectIds = useMemo(
    () =>
      new Set(
        semesterRows.flatMap((row) =>
          (row.subjectAssignments || []).map(
  ...
  const onSubmit = async (values) => {
    try {
      ...
      const assignmentSubjects = semesterSubjectsPayload.flatMap((row) =>
        row.subjectAssignments.map((assignment) => assignment.subject),
      );
      const assignmentFaculty = semesterSubjectsPayload.flatMap((row) =>
  ...
  const getCampusLabel = () => {
    if (isSuperAdmin)
      return (
        campuses.find(
          (campus) => campus.id === (watch("campus") || selectedCampuses[0]),
        )?.name || "Campus"
      );
    const campusId = currentAdmin?.campus?._id || currentAdmin?.campus || "";
      ...
      {loading ? (
        ...
        <>
            ...
            <div className="col-span-1 md:col-span-2">
              ...
              {isSuperAdmin ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {campuses.map((campus) => {
                    const isSelected =
                      (watch("campus") || selectedCampuses[0]) === campus.id;
                    return (
                      <label
                        key={campus.id}
                      ...
                      >
                        ...
                        />
                        <Building2
                          className={`w-6 h-6 mb-2 ${isSelected ? "text-college-navy dark:text-college-gold" : "text-gray-400"}`}
                        />
                        <span
                          className={`text-sm font-bold text-center ${isSelected ? "text-college-navy dark:text-college-gold" : "text-gray-600 dark:text-gray-400"}`}
                        >
                          {campus.name}
                        </span>
                        {isSelected && (
                ...
                <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-college-navy/50 border border-gray-200 dark:border-college-gold/20 rounded-sm text-gray-700 dark:text-gray-200">
                  ...
                  <span className="ml-auto text-xs bg-college-gold/10 text-college-navy dark:text-college-gold px-2 py-1 rounded-full">
                    Fixed Campus
                  </span>
                </div>
              )}
          ...
          <PortalForm.Section title="Class Summary">
            <div className="col-span-1 md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="rounded-sm border border-gray-200 bg-gray-50 p-4 dark:border-college-gold/20 dark:bg-college-navy/40">
                ...
                <p className="mt-1 text-2xl font-bold text-college-navy dark:text-college-gold">
                  {totalStudents}
                </p>
              </div>
              <div className="rounded-sm border border-gray-200 bg-gray-50 p-4 dark:border-college-gold/20 dark:bg-college-navy/40">
              ...
              <div className="rounded-sm border border-gray-200 bg-gray-50 p-4 dark:border-college-gold/20 dark:bg-college-navy/40">
                ...
                <p className="mt-1 text-2xl font-bold text-college-navy dark:text-college-gold">
                  {lockedSemesterCount}/{semesterRows.length || 0}
                </p>
              </div>
            </div>
          </PortalForm.Section>
          ...
          {selectedCourse && selectedCourse.examSystem !== "other" && (
            <PortalForm.Section
              title={`${isAnnualSystem ? "Year" : "Semester"} Subject Allocation`}
            >
              <div className="col-span-1 md:col-span-2 rounded-sm border border-dashed border-gray-300 dark:border-college-gold/20 bg-gray-50/70 dark:bg-college-navy/40 p-4 text-sm text-gray-600 dark:text-gray-300">
                Subjects, teachers, dates, and lock state are managed per term.
                Completed terms stay locked.
              </div>
                    ...
                    <div className="flex items-center justify-between gap-3 mb-4">
                      <div>
                        <h3 className="text-sm font-semibold text-gray-800 dark:text-white">
                          {isAnnualSystem ? "Year" : "Semester"}{" "}
                          {row.semesterNumber}
                        </h3>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {locked
                            ? "Locked because this term is already completed or published"
                            : "Optional term setup"}
                        </p>
                      </div>
                      <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-college-navy/10 text-college-navy dark:text-college-gold">
                        {(row.subjectAssignments || []).length} subjects
                      </span>
                    </div>
                      ...
                      <div className="md:col-span-2 space-y-3">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              Add one subject at a time and choose the teacher
                              for it.
                            </p>
                          </div>
                          <button
                            ...
                            disabled={
                              locked ||
                              (row.subjectAssignments || []).length >=
                                TERM_SUBJECT_LIMIT
                            }
                            onClick={() =>
                              openAssignmentDialog(row.semesterNumber)
                            }
                          ...
                          {(row.subjectAssignments || []).length === 0 ? (
                            ...
                            (row.subjectAssignments || []).map(
                              (assignment, assignmentIndex) => {
                                const subject = visibleSubjects.find(
                                  (item) => item._id === assignment.subject,
                                );
                                const faculty = visibleFacultyUsers.find(
                                  (user) => user._id === assignment.faculty,
                                );
                                return (
                                  <div
                                    key={`${assignment.subject}-${assignmentIndex}`}
                                    className="flex items-center justify-between gap-3 rounded-sm border border-gray-200 dark:border-college-gold/20 bg-white dark:bg-college-navy/50 px-4 py-3"
                                  >
                                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          Subjects selected in other terms are hidden here.
                          Maximum {TERM_SUBJECT_LIMIT} subjects.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="w-full max-w-lg rounded-sm bg-white dark:bg-college-navy border border-gray-200 dark:border-college-gold/20 shadow-xl p-5 space-y-4">
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-sm border border-gray-200 dark:border-college-gold/20 text-sm font-medium text-gray-700 dark:text-gray-300"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-sm bg-college-navy text-white text-sm font-medium"
                  >
                    <Plus className="w-4 h-4" />
                    Add
                  </button>
