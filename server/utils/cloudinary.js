...
const isPlaceholderValue = (value) => {
  ...
  return (
    normalized.includes("replace_api_key") ||
    normalized.includes("replace_api_secret") ||
...
const buildLocalUploadResult = async (
  buffer,
  folder,
  resourceType,
  metadata = {},
  ...
  const fileBaseName =
    path
      .basename(originalName, path.extname(originalName) || undefined)
  ...
  const relativePath = path.posix
    .join(...safeFolder.map((segment) => segment.replace(/\\/g, "/")), fileName)
    .replace(/\\/g, "/");
