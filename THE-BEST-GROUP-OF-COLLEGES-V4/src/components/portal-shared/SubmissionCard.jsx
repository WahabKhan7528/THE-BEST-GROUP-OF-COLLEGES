...
const statusBadge = {
  'On-time': 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-100 dark:border-emerald-700/40',
  Late: 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border-red-100 dark:border-red-700/40',
  Pending: 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-500 border-amber-100 dark:border-amber-800/30',
};
...
const SubmissionCard = ({ submission, role = 'faculty', onGrade }) => {
  ...
  return (
    <Card hover={false} className="p-4 md:p-5 space-y-4 border border-transparent dark:border-college-gold/60 shadow-sm transition-all duration-300">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`inline-flex items-center justify-center text-center shrink-0 whitespace-nowrap px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-bold border uppercase tracking-wider shadow-sm ${badge}`}>
          {submission.status}
        </span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pt-4 border-t border-gray-100 dark:border-white/5">
        ...
        {isStudent ? (
          submission.remarks && (
            <div className="flex-1 max-w-md p-3 rounded-xl bg-college-navy/5 dark:bg-white/5 border border-college-navy/10 dark:border-white/10 ml-0 lg:ml-4">
              <div className="space-y-1">
                <p className="text-[10px] uppercase font-bold text-gray-400 dark:text-gray-500">Instructor Feedback</p>
                <p className="text-xs text-gray-700 dark:text-gray-300 italic">"{submission.remarks}"</p>
              </div>
            </div>
          )
        ) : (
