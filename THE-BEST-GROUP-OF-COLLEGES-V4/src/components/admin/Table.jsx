...
const Table = ({ columns, data, actionButtons }) => {
  const cellPadding = "px-3 md:px-6 py-3 md:py-4";
        ...
        <table className="min-w-full divide-y divide-gray-100 dark:divide-dark-border">
          ...
          <thead>
            <tr className="bg-college-navy/5 dark:bg-college-gold/10">
              {columns.map((col) => (
                <th key={col.key} className={`${cellPadding} text-left text-[10px] md:text-xs font-bold text-college-navy dark:text-college-gold uppercase tracking-wider`}>
                  {col.label}
                </th>
              ))}
              {actionButtons && (
                <th className={`${cellPadding} text-right text-[10px] md:text-xs font-bold text-college-navy dark:text-college-gold uppercase tracking-wider`}>
                  Actions
                </th>
              )}
            </tr>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-dark-border bg-white dark:bg-college-navy">
            {data.map((row, idx) => (
              <tr key={row.id || idx} className="hover:bg-college-navy/5 dark:hover:bg-college-gold/10 transition-colors duration-150 group">
                {/* Data Cells */}
                {columns.map((col) => (
                  <td key={col.key} className={`${cellPadding} whitespace-nowrap text-xs md:text-sm text-gray-700 dark:text-gray-300 group-hover:text-college-navy dark:group-hover:text-college-gold transition-colors`}>
                    {row[col.key]}
                  </td>
                ))}
